...
public class Actuator : WECommand
{
	...
	{
		_remove = remove;
		_expression = expression ?? new TestExpression((ref tile) => true);
    }
	...
	public override void Execute()
	{
		...
		for (int tileX = x; tileX <= x2; tileX++)
		{
			for (int tileY = y; tileY <= y2; tileY++)
			{
				...
				if (tile.active() && (_remove ? tile.actuator() : !tile.actuator()) && select(tileX, tileY, plr) && _expression.Evaluate(ref tile) && magicWand.InSelection(tileX, tileY))
				{
