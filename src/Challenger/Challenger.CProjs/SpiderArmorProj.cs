...
public class SpiderArmorProj : CProjectile
{
    ...
    public override void PreProjectileKilled()
    {
        if (this.proj.type == 249 && this.lable == 2012)
        {
            for (var i = 0; i < 15; i++)
            {
                ...
                var num = Projectile.NewProjectile(this.proj.GetProjectileSource_FromThis(), this.proj.Center, val * 4f, 265, 40, 5f, this.proj.owner);
                Update(num);
            }
            this.lable = 0;
        }
    ...
    {
        var num = Projectile.NewProjectile(Projectile.GetNoneSource(), position, velocity, 249, 20, 0f, owner);
